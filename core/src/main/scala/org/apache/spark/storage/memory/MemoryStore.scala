...
private[spark] class MemoryStore(
    ...
    _rmemStore: Option[DiskStoreLike])
  extends Logging {
  ...
  private val useRmem = _rmemStore.isDefined
  private def rmemStore = {
    assert(_rmemStore.isDefined,
      "Calling RmemStore in MemoryStore when MemoryStore not configured to use it")
    _rmemStore.get
  }
